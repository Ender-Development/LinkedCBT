...
import org.ender_development.linkedcbt.util.EnergyUtils
...
class EnergySideConfiguration(val tile: TileLinkedBattery) : BaseSideConfiguration<IEnergyStorage>() {
	...
	override fun tick() {
		for((facing, side) in sides) {
			...
			if((side == SideConfiguration.PUSH && tile.energyStorage.energyStored == 0) || (side == SideConfiguration.PULL && tile.energyStorage.energyStored ==
                    tile.energyStorage.maxEnergyStored
                ))
				continue
