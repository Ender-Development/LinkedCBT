...
import org.ender_development.linkedcbt.util.FluidUtils
...
class FluidSideConfiguration(val tile: TileLinkedTank) : BaseSideConfiguration<IFluidHandler>() {
	...
	override fun tick() {
		for((facing, side) in sides) {
			...
			if((side == SideConfiguration.PUSH && handler.fluidAmount == 0) || (side == SideConfiguration.PULL && handler.fluidAmount ==
                    handler.capacity
                ))
				continue
