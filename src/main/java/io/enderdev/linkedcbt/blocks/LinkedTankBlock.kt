...
class LinkedTankBlock : BaseRotatableMachineBlock(LinkedCBT, "linked_tank", LinkedCBT.guiHandler.registerId(TileLinkedTank::class.java, ContainerLinkedTank::class.java) { GuiLinkedTank::class.java }) {
	companion object {
		val forceUpdate: PropertyBool = PropertyBool.create("force_update")
	}

	init {
		defaultState = defaultState.withProperty(forceUpdate, false)
	}

	override fun createBlockState() =
		BlockStateContainer(this, BlockHorizontal.FACING, forceUpdate)
	...
	override fun getStateForPlacement(world: World, pos: BlockPos, facing: EnumFacing, hitX: Float, hitY: Float, hitZ: Float, meta: Int, placer: EntityLivingBase, hand: EnumHand): IBlockState =
		super.getStateForPlacement(world, pos, facing, hitX, hitY, hitZ, meta, placer, hand).withProperty(forceUpdate, false)

	@Deprecated("")
	override fun getStateFromMeta(meta: Int): IBlockState =
		@Suppress("DEPRECATION")
		super.getStateFromMeta(meta and 0b0011).withProperty(forceUpdate, meta and 0b0100 != 0)

	override fun getMetaFromState(state: IBlockState) =
		super.getMetaFromState(state) or (if(state.getValue(forceUpdate)) 0b0100 else 0)
